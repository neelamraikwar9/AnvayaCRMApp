...
function onInputChange(e) {
  ...
  if (name === "salesAgent") {
    ...
    const selectedAgent = salesAgents.find(agent => agent._id === value) || null;
    setEdit(prev => ({
      ...prev,
  ...
  return (
    ...
    <div className="midContainer" style={{border: '1px ', width: '55rem', }}>
    <div style={{border: '1px solid white', width: '20rem', height: '22rem', padding: '2rem',  borderRadius: '1rem'}}>
    <p>Lead Name: {lead?.name}</p>
    <p>Lead Source: {lead?.source}</p>
    <p>Sales Agent: {lead?.salesAgent?.name} {" "} ({lead?.salesAgent?.email})</p>
    ...
    <button onClick={() => setFormModel(true)} className="editBtn">Edit</button>
    </div>
