...
const LeadForm = () => {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const response = await axios.get("https://anvaya-model-references-apis-backen.vercel.app/tags");
  ...
  const handleChange = (e) => {
    ...
    if (error.response) {
       console.error("Server error:", error.response.data);
        ...
        alert(` Error: ${error.response.data.message || 'please fill the form.' || 'Failed to add lead'}`);
  ...
  disabled={loadingAgents}
        >
          <option value="">{loadingAgents ? 'Loading Agents...' : 'Select Sales Agent'}</option>
        ...
        <button type="submit"  disabled={isSubmitting} className="button">{ isSubmitting ? 'Create Lead' : 'Create Lead'}</button>
        </div>
      </form>
      </div>
    </main>
